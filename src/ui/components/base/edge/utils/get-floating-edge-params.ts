...
function getHandleCoordsByPosition(node: Node, handlePosition: Position) {
	if (!node.positionAbsolute || !node.width || !node.height) {
		console.error('Node is unstable', !node.positionAbsolute, !node.width, !node.height);
		throw new Error('Node is unstable');
	}
	...
	if (!handle) {
		console.error('No handle found', node, handlePosition);
